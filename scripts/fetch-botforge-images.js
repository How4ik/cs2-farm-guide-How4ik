...
const BASE = 'https://pochito.gitbook.io/bot-forge';
const LLMS = `${BASE}/llms.txt`;
const SPACE_MARKER = '/spaces/';
      ...
      .get(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          const next = res.headers.location.startsWith('http')
            ...
            : `https://pochito.gitbook.io${res.headers.location}`;
...
function pageUrlsFromLlms(llms) {
  return [...llms.matchAll(/\((https:\/\/pochito\.gitbook\.io\/bot-forge\/[^)]+\.md)\)/g)].map((m) => m[1]);
}

function contentImages(html) {
  const proxies = [...html.matchAll(/https:\/\/pochito\.gitbook\.io\/bot-forge\/~gitbook\/image\?url=[^"'\\]+/g)].map(
  ...
  for (const proxy of proxies) {
    ...
    if (!decoded.includes(SPACE_MARKER) || !decoded.includes('/uploads/')) continue;
...
module.exports.run = async () => {
  ...
  const outDir = path.join(root, 'chapters', 'images', 'bot-forge');
  ...
  for (const [id, proxy] of Object.entries(proxies)) {
    ...
    const rel = `images/bot-forge/${id}${ext}`;
